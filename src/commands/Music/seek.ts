	...
	public async messageRun(message: Message, args: Args) {
		...
		try {
			...
			if (!formatValidator.test(seekAmmount)) {
				embedReply.setDescription(
					'Invalid time format!, usage: `%seek hh:mm:ss`, `%seek mm:ss`, `%seek ss`'
				);
				return message.reply({ embeds: [embedReply] });
			}
			...
			if (seekSeconds > erelaPlayer.queue.current?.duration! / 1000 - 1) {
				embedReply
					.setDescription(
						"The specified time is longer that the current track's length!"
					)
					.setColor('RED');
			...
			if (error.identifier === 'argsMissing') {
				embedReply.setDescription(
					'You must specify the ammount of time to seek!'
				);
				return message.reply({ embeds: [embedReply] });
			}
